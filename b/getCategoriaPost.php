...
<body>
    ...
    if (isset($_SESSION['email'])) {
        ...
        try {
            ...
            include('config.php');
			$_SESSION['id_categoria']=$get_id;

            // CONSULTA SQL

            $sql_consult = "SELECT * FROM post p INNER JOIN post_categorias pc INNER JOIN categorias c WHERE p.id_post=pc.id_post and pc.id_categoria=c.id_categoria and c.id_categoria='$get_id'";
			...
			$sql_consult1 = "SELECT * FROM categorias WHERE id_categoria='$get_id'";

            // PREPARA LA CONSULTA SQL, LA EJECUTA Y CUENTA LOS RESULTADOS

            $resultat1 = $connection->prepare($sql_consult1);

            $resultat1->execute();

            $cat = $resultat1->rowCount();
                    ...
                    <div class="container">
						<?php
							foreach($resultat1 as $resultats1){
								?>
						 <h1 class="text-center">Administración de la categoria: <?php echo $resultats1['descr_categoria']; ?> </h1><br>
						<?php
								}
						?>
						<div class="d-flex" style="margin-left:390px">
                        
                        <div class="d-flex justify-content-center">
                      <a href="admin3"><button class=" btn btn-outline-secondary btn-lg consult-categorias">Volver a todas las categorias</button></a>&nbsp;
                        </div><br>
                                        ...
                                        ?>
                                            <tr>
												...
												<td> <?php echo $resultats['orden_p_c'] ?></td>
												  ...
												  <td><a href="edit_p_c?id_cat=<?php echo urlencode($get_id); ?>&id_post=<?php echo urlencode($resultats['id_post']); ?>"> <button title="Leer y editar post" class=" btn btn-success editar "><i class="bi bi-book"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-book" viewBox="0 0 16 16">
